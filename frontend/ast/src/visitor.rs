...
pub trait Visitor {
	...
	fn visit_init_val_list(&mut self, node: &mut InitValList) -> Result<AstRetType>;
	fn visit_literal_int(&mut self, node: &mut LiteralInt) -> Result<AstRetType>;
	fn visit_literal_float(&mut self, node: &mut LiteralFloat) -> Result<AstRetType>;
	...
	fn visit_formal_param(&mut self, node: &mut FormalParam) -> Result<AstRetType>;
