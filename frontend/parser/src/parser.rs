...
lazy_static::lazy_static! {
    static ref PRATT_PARSER: PrattParser<Rule> = {
        ...
        PrattParser::new()
            ...
            .op(
                Op::infix(Rule::Mul, Left) | Op::infix(Rule::Div, Left) | Op::infix(Rule::Mod, Left)
            )
            .op(
                Op::prefix(Rule::UnaryAdd) |
                    Op::prefix(Rule::UnarySub) |
                    Op::prefix(Rule::UnaryNot) |
                    Op::prefix(Rule::UnaryBitNot)
            )
    };
}
...
fn parse_expr(pair: Pair<Rule>) -> Node {
    PRATT_PARSER.map_primary(parse_primary_expr)
...
fn parse_var_def(pair: Pair<Rule>) -> Node {
    ...
    for pair in pairs {
        match pair.as_rule() {
            Rule::DimList => {
                var_def.dim_list = parse_dim_list(pair);
            }
            Rule::Expr => {
                var_def.init = Some(parse_init_val(pair));
            }
            Rule::InitValList => {
                var_def.init = Some(parse_init_val(pair));
            }
...
pub fn parse(str: &str) -> Result<Program> {
    let pairs = SysycParser::parse(Rule::Program, str).map_err(|e| LexError(e.to_string()))?;
