...
fn move_local_array(program: &mut LlvmProgram) -> Result<bool> {
	...
	program.funcs.iter_mut().filter(|func| func.name == "main").for_each(
		|func| {
			let new_name = |t: &LlvmTemp| {
				format!("__optimized_local_array_{}_{}", func.name, t.name)
			};

			for block in &func.cfg.blocks {
				let mut unkill_size = 0;

				let new_instrs: Vec<LlvmInstr> =
					std::mem::take(&mut block.borrow_mut().instrs)
						.into_iter()
						.map(|instr| match instr.get_variant() {
							LlvmInstrVariant::AllocInstr(i) => match i.length {
								Value::Int(size) if size > 4 => {
...
fn move_global_scalar(program: &mut LlvmProgram) -> Result<bool> {
	...
	let (retained_global, killed_global): (Vec<_>, Vec<_>) =
		std::mem::take(&mut program.global_vars)
			.into_iter()
			.partition(|var| var.is_array || not_appliable.contains(&var.ident));

	program.global_vars.extend(retained_global);
