...
pub fn get_ast_node(
	...
	if let Value::Temp(t) = val {
		if let Some(ast_node) = ast_map.get(t) {
			...
			Rc::new(RefCell::new(AstNode::Value(Value::Int(0))))
		}
	} else {
...
pub fn map_ast_instr(
	...
	match instr.get_variant() {
		ArithInstr(arith_instr) => {
			...
			let res = Rc::new(RefCell::new(AstNode::Expr((
				rs1,
				LlvmOp::ArithOp(arith_instr.op),
				rs2,
			))));
		...
		CallInstr(call_instr) => {
			...
			let res = Rc::new(RefCell::new(AstNode::CallVal(
				call_instr.func.to_string().clone(),
				args,
			)));
