...
impl<'a> IndVarSolver<'a> {
    ...
    pub fn get_loop_info(&mut self) -> Option<LoopInfo> {
        ...
        let single_exit = match self.cur_loop.borrow().get_single_exit(&self.cur_loop.borrow().blocks_without_subloops(&self.cfg, self.loop_map), self.loop_map) {
            Some(bb) => bb,
            _ => return None
        };
        ...
        if let Some(jump_instr) = header_borrowed.jump_instr.as_ref() {
            match jump_instr.get_variant() {
                LlvmInstrVariant::JumpCondInstr(cond_inst) => {
                    ...
                    let def_branch_temp = self.temp_graph.temp_to_instr[&branch_temp].instr.clone();
                    match def_branch_temp.get_variant() {
                        LlvmInstrVariant::CompInstr(inst) => {
                            if matches!(inst.op, CompOp::SLT | CompOp::SLE | CompOp::SGT | CompOp::SGE) {
                                let mut get_info = |cond_value: Value, end_value: Value, take_reverse: bool| -> Option<LoopInfo> {
                                    if self.is_loop_invariant(&end_value) {
                                        if let Some(t) = cond_value.unwrap_temp() {
                                            if let Some(iv) = self.indvars.get(&t).cloned() {
                                                let new_op = convert_comp_op(inst.op, take_not, take_reverse);
                                                let loop_cnt = self.compute_loop_cnt(iv.base.clone(), iv.step.clone(), end_value.clone(), new_op);
    ...
    pub fn compute_loop_cnt(&mut self, start: Value, step: Value, end: Value, op: CompOp) -> Value {
        match op {
            CompOp::SLT | CompOp::SGT => {
                // (end - start + step - 1) / step;
                let tmp1 = self.compute_two_value(end.clone(), start.clone(), llvm::ArithOp::Sub);
                let tmp2 = self.compute_two_value(tmp1, step.clone(), llvm::ArithOp::Add);
                let tmp3 = self.compute_two_value(tmp2, llvm::Value::Int(1), llvm::ArithOp::Sub);
                let tmp4 = self.compute_two_value(tmp3, step.clone(), llvm::ArithOp::Div);
                tmp4
            }
            CompOp::SLE | CompOp::SGE => {
                ...
                let tmp1 = self.compute_two_value(end.clone(), start.clone(), llvm::ArithOp::Sub);
                let tmp2 = self.compute_two_value(tmp1, step.clone(), llvm::ArithOp::Add);
                let tmp3 = self.compute_two_value(tmp2, step.clone(), llvm::ArithOp::Div);
                tmp3
            }
            _ => unreachable!()
        }
...
fn convert_comp_op(mut op: CompOp, take_not: bool, take_reverse: bool) -> CompOp {
    assert!(matches!(op, CompOp::SLT | CompOp::SLE | CompOp::SGT | CompOp::SGE));
    op = if take_not {
        match op {
