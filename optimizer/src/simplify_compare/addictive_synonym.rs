	...
	> AddictiveSynonym<T, K>
{
	...
	pub fn get(&self, query: &K) -> Option<(K, T)> {
		if let Some(catagory) = self.catagory.get(&query) {
			let (repr, map) = &self.offsets[catagory];
			Some((repr.clone(), map[query]))
		} else {
			None
		}
	}
...
impl LlvmTempAddictiveSynonym {
	...
	pub fn get(&self, t: &LlvmTemp) -> Option<(LlvmTemp, Value)> {
		match t.var_type {
			llvm::VarType::I32 => self.int.get(t).map(|(t, v)| (t, v.into())),
			// llvm::VarType::F32 => {
			//TODO
			// }
			_ => None,
